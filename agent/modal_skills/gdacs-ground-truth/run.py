...
evaluator scores forecasts against. By default keeps tropical-cyclone
...
def run(now: datetime, db: Connection) -> int:
    ...
    if not rows:
        print(
            f"[{SKILL_ID}] no GDACS events matched types {sorted(TYPES)} "
            f"(feed had {len(items)} items)."
        )
        return 0
